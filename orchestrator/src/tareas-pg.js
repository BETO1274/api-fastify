...
const COLUMNAS = 'id, servicio, metodo, ruta, body, estado, resultado, creada_en'
...
export async function crearTarea({ servicio, metodo, ruta, body }) {
  const { rows } = await obtenerPool().query(
    `insert into orquestador_tarea (id, servicio, metodo, ruta, body)
     values ($1, $2, $3, $4, $5)
     returning ${COLUMNAS}`,
    [crypto.randomUUID(), servicio, metodo, ruta, body === undefined ? null : JSON.stringify(body)]
  )
