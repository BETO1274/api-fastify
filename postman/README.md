...
Suite de validación end-to-end contra los ambientes reales desplegados (Test y Producción).
...
* `api-fastify-test.postman_environment.json` — `base_url` → Test.
* `api-fastify-produccion.postman_environment.json` — `base_url` → Producción.

## Cómo correrla

1. Importar los 3 archivos en Postman (o usar `newman`, ver abajo).
